     <template>
       ...
       <div class="jumbotron jumbotron-fluid vertical-center text-center">
            <div class="container">
            <h1 class="display-4" style="color:white;">What's for Dinner?</h1>
            <!-- <hr class="my-4"> -->
            <p>It's the age-old question. Search to find out.</p>
                <!-- Form for search field and button -->
                <form class="form-check form-check-inline">
                    <input class="form-control mr-sm-2" v-model="search" type="search" placeholder="Search for Food" aria-label="Search" id="search-input">
                    <button @click="fetchData(search)" class="btn my-2 my-sm-0" type="button" id="search-button" ><i class="fas fa-search"></i></button>
                </form>
...
<!-- 
        ...
        import MealCard from "./MealCard.vue"
            ...
            methods: {
                fetchData(search) {
                    axios.get(`https://api.spoonacular.com/recipes/search?apiKey=52e7e5abc8da43e3b0722667e3cec54d&query=`+search)
                        .then(res => {
